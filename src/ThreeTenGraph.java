...
class ThreeTenGraph implements Graph<GraphNode,GraphEdge>, UndirectedGraph<GraphNode,GraphEdge> {
    ...
    public boolean containsVertex(GraphNode vertex) {
		//O(1)
		return storage[vertex.id][vertex.id] != null;
	}
    ...
    public Pair<GraphNode> getEndpoints(GraphEdge edge) {
		...
		for (int i = 0; i < storage.length; i++)
		{
			for (int j = 0; j < storage[i].length; j++)
			{
				if (i != j && storage[i][j] != null && storage[i][j].equals(edge))
				{
					n1 = new GraphNode(i);
					n2 = new GraphNode(j);
				}
    ...
    public boolean addEdge(GraphEdge e, GraphNode v1, GraphNode v2) {
		storage[v1.id][v2.id] = e;
		storage[v2.id][v1.id] = e;

		//todo we need to check that the vertex is not already connected
		return true;
	}
    ...
    public boolean addVertex(GraphNode vertex) {
		if (vertex != null && storage[vertex.id][vertex.id] == null)
		{
			storage[vertex.id][vertex.id] = vertex;
			return true;
		}
...
//		return null;
	...
	private String dfsHelper(GraphNode curr, boolean[] visited)
	{
		...
		if (curr == null)
		{
			for (int i = 0; i < visited.length; i++)
			{
				//GraphNodes are located on the diagonals
				if (!visited[i] && storage[i][i] != null)
				{
		...
		for (int i = 0; i < storage.length; i++)
		{
			if (curr.id != i)
			{
				if (storage[i][curr.id] != null)
				{
					GraphEdge currEdge = (GraphEdge) storage[i][curr.id];

					Pair<GraphNode> nodes = getEndpoints(currEdge);
					GraphNode v1 = nodes.getFirst();
					if (!visited[v1.id])
					{
						visited[v1.id] = true;
